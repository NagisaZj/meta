...
class network(nn.Module):
    ...
    def surrogate_loss(self,episodes,adapt_params,old_pis=None):
        losses, kls, pis = [], [], []
        if old_pis is None:
            old_pis = [None] * len(episodes)

        for valid_episodes, old_pi,params in zip(episodes, old_pis,adapt_params):
    ...
    def hessian_vector_product(self, episodes, adapt_params,damping=1e-2):
        """Hessian-vector product, based on the Perlmutter method."""

        def _product(vector):
            kl = self.kl_divergence(episodes,adapt_params)
            grads = torch.autograd.grad(kl, self.parameters(),
                                        create_graph=True)
            flat_grad_kl = parameters_to_vector(grads)

            grad_kl_v = torch.dot(flat_grad_kl, vector)
            grad2s = torch.autograd.grad(grad_kl_v, self.parameters())
    ...
    def kl_divergence(self, episodes, adapt_params,old_pis=None):
        kls = []
        if old_pis is None:
            old_pis = [None] * len(episodes)
...
def test(learner,args,train_envs, test_envs,log_dir):
    ...
    for i in range(args.num_updates):
        ...
        print(np.mean(rew_rem))
        old_loss, _, old_pis = learner.surrogate_loss(adapt_episodes,adapt_params)
        grads = torch.autograd.grad(old_loss, learner.parameters(),retain_graph=True)
        grads = parameters_to_vector(grads)

        # Compute the step direction with Conjugate Gradient
        hessian_vector_product = learner.hessian_vector_product(adapt_episodes,adapt_params,
                                                             damping=cg_damping)
        ...
        for _ in range(ls_max_steps):
            vector_to_parameters(old_params - step_size * step,
                                 learner.parameters())
            loss, kl, _ = learner.surrogate_loss(adapt_episodes, adapt_params,old_pis=old_pis)
...
if __name__=="__main__":
    ...
    parser.add_argument('--variance', help='std for reward sampling', type=float, default=0.3)
    ...
    parser.add_argument('--dir', help='log directory', type=str,default='./data/rl_toy/')
    parser.add_argument('--number', help='number for storing data', type=int, default=5)
    parser.add_argument('--batch_size', help='batch size(also episode length)', type=int, default=20)

    args = parser.parse_args()
    random.seed(0)
    log_dir=args.dir+str(args.num_layers)+'_'+str(2)+'_'+str(args.inner_lr)+'_'+str(
        args.outer_lr)+'_'+str(args.num_bandits)+'_'+str(args.r_positive)+'_'+str(args.r_negative)+'_'+str(args.batch_size)+'_'+str(args.variance)+'clip1ttt/'
